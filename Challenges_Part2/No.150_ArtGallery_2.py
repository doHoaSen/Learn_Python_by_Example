...
def search_artist():
    ...
    cursor.execute("SELECT Artists.name FROM Artists WHERE artistid = ?", [select_artist])
    for x in cursor.fetchall():
        newrecord = str(x[0]) + ", " + str(x[1]) + ", " + str(x[2]) + ", " + str(x[3]) + ", " + str(x[4]) + "\n"
        outputwindow.insert(END, newrecord)
...
def search_by_price():
    ...
    cursor.execute("""SELECT Art.pieceid, Artists.name, \
        FROM Artists, Art WHERE Artist.artistid = Art.artistid AND Art.price >= ? AND \
